...
public class BikeDAOImpl extends BaseDAOImpl<BikeDTO> implements BikeDAOInt {
	...
	@Override
	protected void populate(BikeDTO dto, UserContext userContext) {
		if (dto.getPreloadId() != null && dto.getPreloadId() > 0) {
			PreloadDTO categoryDto = preloadDao.findByPK(dto.getPreloadId(), userContext);
			dto.setPreloadName(categoryDto.getName());
			System.out.println(dto.getPreloadName() + "PriorityName-------");
		}

	}
	@Override
	protected List<Predicate> getWhereClause(BikeDTO dto, CriteriaBuilder builder, Root<BikeDTO> qRoot) {
