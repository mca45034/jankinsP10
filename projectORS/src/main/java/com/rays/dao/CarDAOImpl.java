...
public class CarDAOImpl extends BaseDAOImpl<CarDTO> implements CarDAOInt{
	...
	@Override
	protected void populate(CarDTO dto, UserContext userContext) {
	...
	protected List<Predicate> getWhereClause(CarDTO dto, CriteriaBuilder builder, Root<CarDTO> qRoot) {
		...
		if (!isZeroNumber(dto.getPreloadId())) {
			whereCondition.add(builder.equal(qRoot.get("genderId"), dto.getPreloadId()));
		}

		if (!isEmptyString(dto.getPreloadName())) {
			whereCondition.add(builder.like(qRoot.get("genderName"), dto.getPreloadName() + "%"));
		}
